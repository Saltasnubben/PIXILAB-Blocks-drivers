...
define(["require", "exports", "system/Network", "system_lib/Driver", "system_lib/Metadata", "system_lib/Metadata"], function (require, exports, Network_1, Driver_1, Meta, Metadata_1) {
    ...
    var LevelControl = /** @class */ (function () {
        function LevelControl(owner, channelNumber, // 1-based channel number
        nrpnMSB, nrpnLSBBase, propertyPath // e.g., "channelLevel"
        ) {
            this.owner = owner;
    ...
    var MuteControl = /** @class */ (function () {
        function MuteControl(owner, channelNumber, // 1-based channel number
        nrpnMSB, nrpnLSBBase, propertyPath // e.g., "channelMute"
        ) {
            this.owner = owner;
            this.channelNumber = channelNumber;
            this.nrpnMSB = nrpnMSB;
    ...
    var AllenHeath_SQ = /** @class */ (function (_super) {
        ...
        Object.defineProperty(AllenHeath_SQ.prototype, "lrLevel", {
            ...
            set: function (value) {
                ...
                var nrpnValue = Math.round(normalizedLevel * 16383);
                console.info("Set Main LR level to ".concat(value.toFixed(1), " dB"));
        ...
        AllenHeath_SQ.prototype.processNRPN = function () {
            ...
            if (this.nrpnMSB === 0x4F) {
                ...
                if (this.nrpnLSB < 48) {
                    ...
                    console.info("Channel ".concat(channelNumber, " level feedback: ").concat(levelDB.toFixed(1), " dB"));
                    this.channelLevel[channelNumber].updateValue(levelDB);
                    this.changed("channelLevel[".concat(channelNumber, "]"));
                }
                // DCA levels: LSB = 0x20-0x27 (32-39) for DCA 1-8
                    ...
                    console.info("DCA ".concat(dcaNumber, " level feedback: ").concat(levelDB.toFixed(1), " dB"));
                    this.dcaLevel[dcaNumber].updateValue(levelDB);
                    this.changed("dcaLevel[".concat(dcaNumber, "]"));
                }
                // Main LR level: LSB = 0x30 (48)
                else if (this.nrpnLSB === 0x30) {
                    console.info("Main LR level feedback: ".concat(levelDB.toFixed(1), " dB"));
                ...
                // Mix levels: LSB = 0x31-0x3C (49-60) for Mix 1-12
                    ...
                    console.info("Mix ".concat(mixNumber, " level feedback: ").concat(levelDB.toFixed(1), " dB"));
                    this.mixLevel[mixNumber].updateValue(levelDB);
                    this.changed("mixLevel[".concat(mixNumber, "]"));
                }
            }
            // Channel mutes: MSB = 0, LSB = channel (0-47)
                ...
                console.info("Channel ".concat(channelNumber, " mute feedback: ").concat(muted));
                this.channelMute[channelNumber].updateValue(muted);
                this.changed("channelMute[".concat(channelNumber, "]"));
            }
            // Main LR and Mix mutes: MSB = 1
                ...
                if (this.nrpnLSB === 0x30) {
                    console.info("Main LR mute feedback: ".concat(muted));
                    this._lrMute = muted;
                    this.changed('lrMute');
                }
                // Mix mutes: LSB = 0x31-0x3C (49-60) for Mix 1-12
                    ...
                    var mixNumber = (this.nrpnLSB - 0x31) + 1; // Convert to 1-based
                    console.info("Mix ".concat(mixNumber, " mute feedback: ").concat(muted));
                    this.mixMute[mixNumber].updateValue(muted);
                    this.changed("mixMute[".concat(mixNumber, "]"));
                }
            }
            // DCA mutes: MSB = 2, LSB = DCA (0-7)
                ...
                var muted = nrpnValue > 0;
                console.info("DCA ".concat(dcaNumber, " mute feedback: ").concat(muted));
                this.dcaMute[dcaNumber].updateValue(muted);
                this.changed("dcaMute[".concat(dcaNumber, "]"));
            }
            // Reset NRPN state
            this.nrpnMSB = -1;
            this.nrpnLSB = -1;
