...
export class KNXNetIP extends Driver<NetworkUDP> {
	...
	// Properties for received KNX messages
	private mLastReceivedSource = "";
	private mLastReceivedDestination = "";
	private mLastReceivedValue: any = null;
	private mLastReceivedRaw = "";
	...
	@property("Last received source address (e.g., 1.2.3)", true)
	get lastReceivedSource(): string {
		return this.mLastReceivedSource;
	}

	@property("Last received destination address (e.g., 4/0/1)", true)
	get lastReceivedDestination(): string {
		return this.mLastReceivedDestination;
	}

	@property("Last received value (boolean or number)", true)
	get lastReceivedValue(): any {
		return this.mLastReceivedValue;
	}

	@property("Last received raw data as hex string", true)
	get lastReceivedRaw(): string {
		return this.mLastReceivedRaw;
	}
	...
	/**
	 * Process incoming tunnel request from KNX gateway (KNX bus messages).
	 * Parse the cEMI frame and expose the data to Blocks.
	 */
	private gotTunnelRequest(packet: number[]) {
		debugLog("gotTunnelRequest", packet.length);

		// Always acknowledge the tunnel request
		this.sendTunnelAck(packet[7], packet[8]);

		// Parse cEMI frame if packet is long enough
		if (packet.length < 20) {
			debugLog("Tunnel request too short to parse");
			return;
		}

		try {
			// cEMI frame starts at byte 10
			const messageCode = packet[10];
			const addInfoLen = packet[11];
			const cemiStart = 10 + 2 + addInfoLen; // Skip message code, add info length, and any add info

			if (packet.length < cemiStart + 8) {
				debugLog("Packet too short for cEMI data");
				return;
			}

			// Parse source address (individual address format: area.line.device)
			const srcAddrRaw = get16bit(packet, cemiStart + 4);
			const srcArea = (srcAddrRaw >> 12) & 0x0F;
			const srcLine = (srcAddrRaw >> 8) & 0x0F;
			const srcDevice = srcAddrRaw & 0xFF;
			this.mLastReceivedSource = `${srcArea}.${srcLine}.${srcDevice}`;

			// Parse destination address (group address format: main/middle/sub)
			const destAddrRaw = get16bit(packet, cemiStart + 6);
			const destMain = (destAddrRaw >> 11) & 0x1F;
			const destMiddle = (destAddrRaw >> 8) & 0x07;
			const destSub = destAddrRaw & 0xFF;
			this.mLastReceivedDestination = `${destMain}/${destMiddle}/${destSub}`;

			// Parse data
			const dataLen = packet[cemiStart + 8];
			...
			if (dataLen === 1) {
				// Single byte with value in lower 6 bits (typical for on/off)
				const dataByte = packet[cemiStart + 10];
				if ((dataByte & 0x80) === 0x80) {
					// On/Off command
					this.mLastReceivedValue = (dataByte & 0x01) === 0x01;
				} else {
					// Small value (0-63)
					this.mLastReceivedValue = dataByte & 0x3F;
				}
			} else if (dataLen === 2) {
				// Two data bytes - typically a number (scene, percentage, etc.)
				this.mLastReceivedValue = packet[cemiStart + 11];
			} else if (dataLen > 2) {
			...
			debugLog("Received KNX message:", {
				...
				destination: this.mLastReceivedDestination,
				value: this.mLastReceivedValue,
				raw: this.mLastReceivedRaw
			});

			// Notify property changes to Blocks
			this.changed('lastReceivedSource');
			this.changed('lastReceivedDestination');
			this.changed('lastReceivedValue');
			this.changed('lastReceivedRaw');

		} catch (error) {
			console.error("Error parsing tunnel request:", error);
		}
	}
